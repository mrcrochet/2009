...
Each item maps to an automated test. `E` = `tests/e2e/case001.spec.ts`, `U` = `tests/unit/*`.
...
7. `E` Relay Mail, Devices, Orbit and the handset overlay all open from the dock.
7b. `E` The handset is locked, and opens only with the passcode recovered from the laptop image.
7c. `E` The forensic recovery offer says the purchase is real and takes no money inside the fiction.
