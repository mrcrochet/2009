...
- `profiles`, `investigations`, `investigation_events` — a player reads and writes only their own rows.
...
limits: `investigations.snapshot` is capped at 512 KB by a check constraint, `investigation_events` has no
